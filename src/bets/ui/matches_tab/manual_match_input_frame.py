...
class ManualMatchInputFrame(LabelFrame):
    ...
    def create_widgets(self):
        ...
        self.title_input = Entry(self, width=constants.W_MATCH_TITLE, textvariable=self._var_title)
        self.title_input.grid(column=1, row=0, columnspan=2)
        self.title_input.focus()

        self.ratio_1_input = Entry(self, width=constants.W_MATCH_RATIO, textvariable=self._var_ratio_1)
        self.ratio_1_input.grid(column=1, row=1)

        self.ratio_x_input = Entry(self, width=constants.W_MATCH_RATIO, textvariable=self._var_ratio_x)
        self.ratio_x_input.grid(column=1, row=2)

        self.ratio_2_input = Entry(self, width=constants.W_MATCH_RATIO, textvariable=self._var_ratio_2)
        ...
        for child in self.winfo_children():
            child.grid_configure(padx=4, pady=2, sticky="WE")
